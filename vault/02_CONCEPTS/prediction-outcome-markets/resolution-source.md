...
**行业正在按"数据源可自动化程度"分流裁决方式**，这是 2025–2026 年最重要的结构性变化：

- **可量化 / 有权威喂价的**（加密价格、指数点位）→ [[polymarket]] 自 **2025-09** 起把价格类合约的裁决改走 [[chainlink]]，**完全绕开人工投票**。
- **需要解释的**（政治、人事、语义性事件）→ 仍走 [[uma]] 乐观预言机或平台内部团队。

**这个分流本身就说明了行业的判断：能自动化的部分，就不要留给人和投票。** 剩下不能自动化的那部分，才是真正的未解难题 —— 也是这个赛道最大的机会所在。
...
- **2026-08-27** — 语义关联层判断 (Claude seed, 待 Irene 复核): 前置 = resolution; typed 关系 1 条。词表见 [[relationship-types|关系类型受控词表]]。
