---
id: "person:jeff-yan"
type: person
title: Jeff Yan
title_zh: 徐冲 (Jeff Yan)
aliases:
  - chameleon_jeff
status: verified
importance: tier-1
domains:
  - people-networks
  - prediction-outcome-markets
tags:
  - person
created: 2026-08-26
updated: 2026-08-26
last_verified: 2026-08-26
review_after: 2027-02-26
confidence: high
epistemic_status: confirmed
confidentiality: public-source
sources:
  - "report:2026-08-26-prediction-venues"
related:
  - id: "venue:hyperliquid-hip4"
    rel: co-founded
    note: Hyperliquid 联合创始人; HIP-4 是其结果市场扩展
---

# Jeff Yan

## Executive Summary

Hyperliquid 联合创始人 (团队唯一实名者), Harvard 数学+CS, 前 Hudson River Trading 高频开发; 零 VC 自融资把链上 CLOB 做成 perp DEX 霸主, 2026 年以 HIP-4 进入结果市场。**早在 ~2018 就做过预测市场项目 "Deaux"** (单源) — 其对结果市场的兴趣先于 Hyperliquid 本体。

## Current Roles (核实 2026-08-26)

| 职务 | 组织 | last_verified | Source | Confidence |
|---|---|---|---|---|
| Co-founder (领导 Hyperliquid Labs; "CEO" 头衔无一手确认) | Hyperliquid / [[hyperliquid-hip4]] | 2026-08-26 | datawallet + hyperliquidguide (二手一致) | medium-high |

## Career Timeline (CONFIRMED 主线)

Palo Alto High → 物理奥赛 2012 银/2013 金 → Harvard 数学+CS (2013-17) → Hudson River Trading → "Deaux" 预测市场项目 (~2018, 单源) → Chameleon Trading (crypto 做市) → 2022 转做 Hyperliquid (零 VC) → 2024-11 HYPE airdrop (31% 供应)。联创: 化名 iliensinc (Harvard 同学)。

## Public Positions

反 VC-token 模式; 产品先于宣传; HFT 哲学直接塑造链上 CLOB 设计。财富估算分歧巨大 (UNVERIFIED, 不采任何数字)。


<!-- timeline -->

## Timeline

- **2026-08-26** — 建页 (web 核验, 证据见 [[report-2026-08-26-prediction-venues]])。
- **2026-09-01** — 实体语义关联层: 依实体页已有 CONFIRMED 事实补 1 条 typed 关系 (词表见 [[relationship-types|关系类型受控词表]]); 证据为本页来源, 未新增断言。
