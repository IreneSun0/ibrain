...
它同时是一门**课**：81 关主线，由目标能力集反向求前置闭包得出，按拓扑序分成 9 章。**81 关全部写到完整深度** —— 每一关都有机制拆解、带数字的具体例子、常见误解、实战检查清单和自测题。[站点](https://irenesun0.github.io/ibrain)渲染的就是这个。
...
每一页的 frontmatter 和图谱导出里都能看到笔记成熟度：110 reviewed、85 verified、99 seed、3 stale。

**81 关主线已全部完成** —— 每个概念都有机制、带数字的例子、常见误解、实战清单和自测题（实质正文中位数约 2,000 字符）。62 个支线概念结构完整且有来源但仍较短，是下一步的扩写对象。**没有任何内容被呈现得比它实际更确定** —— 这正是分档存在的意义。
